...
SAMPLE_SIZE = 100000         # 采样数据量（从65万采样到10万）
SAMPLE_RATIO = None          # 或使用比例采样（例如 0.3 表示30%）

ENABLE_TUNING = True         # 是否启用自动调参
N_ITER_SEARCH = 10           # RandomSearch迭代次数（减少可加速）
CV_FOLDS = 2                 # 交叉验证折数（减少可加速）
...
MODELS_TO_TEST = {
    ...
    'Random Forest': True,
    'Extra Trees': True,
    'Gradient Boosting': True,      # 较慢
    'HistGradient Boosting': True,   # 快速的Boosting替代
    'AdaBoost': True,               # 较慢
}
...
print(f"  - 自动调参: {'启用' if ENABLE_TUNING else '禁用'}")
if ENABLE_TUNING:
    print(f"  - 搜索迭代: {N_ITER_SEARCH} 次")
...
if ENABLE_TUNING and len(baseline_models) > 0:
    # 选择表现最好的前2个模型进行调参（减少数量加速）
    top_n = min(2, len(baseline_df))
    top_models = baseline_df.head(top_n).index.tolist()
    print(f"\n选择表现最好的{top_n}个模型进行超参数优化: {top_models}")
    
    print("\n" + "="*60)
    print("第二阶段：超参数自动调优 (使用 RandomizedSearchCV)")
    print("="*60)
    
    # 🚀 加速策略3: 精简参数搜索空间
    param_grids = {
        'Decision Tree': {
            'regressor__max_depth': [10, 20, 30, 50, 100],
        ...
        'AdaBoost': {
            'regressor__n_estimators': [30, 50, 100, 150, 200, 250, 300],
    ...
    for i, model_name in enumerate(top_models, 1):
        print(f"\n{'='*60}")
        print(f"[{i}/{len(top_models)}] 调优模型: {model_name}")
        print(f"{'='*60}")
        ...
        # 🔧 对于需要密集矩阵的模型，使用密集版本的 preprocessor
        ...
        if not param_grid:
            print(f"⚠️  {model_name} 没有定义参数网格，跳过调优")
            continue
        
        # 使用 RandomizedSearchCV
        print(f"开始随机搜索 (n_iter={N_ITER_SEARCH}, cv={CV_FOLDS})...")
        random_search = RandomizedSearchCV(
            pipe,
            param_distributions=param_grid,
            n_iter=N_ITER_SEARCH,    # 可配置的迭代次数
            cv=CV_FOLDS,             # 可配置的交叉验证折数
            scoring='r2',
            n_jobs=-1,               # 并行计算
            random_state=42,
            verbose=0                # 减少输出
        ...
        tuned_pipelines[f"{model_name} (Tuned)"] = random_search.best_estimator_
...
if importance_available:
    ...
    print(f"\n无法生成特征重要性图表: 所有模型均不支持 feature_importances_")
...
with open(txt_filename, 'w', encoding='utf-8') as f:
    ...
    f.write(f"  - 自动调参: {'启用' if ENABLE_TUNING else '禁用'}\n")
    if ENABLE_TUNING:
        f.write(f"  - 搜索迭代: {N_ITER_SEARCH}\n")
        f.write(f"  - 交叉验证: {CV_FOLDS}折\n")
    f.write("\n")
